...
export default function LoginScreen() {
  ...
  const ensureVisible = useCallback((ref: React.RefObject<any>, attempt: number = 0) => {
    ...
    setTimeout(() => {
      const node0 = ref?.current as any;
      if (!node0 || typeof node0.measureInWindow !== 'function') {
        // If we can't measure, do a gentle fallback on the last attempt
        if (attempt >= 3) {
      ...
      node0.measureInWindow((x: number, y: number, width: number, height: number) => {
        ...
        if (attempt < 3) {
          ...
          // Final verification + fallback with defensive null checks
          setTimeout(() => {
