...
export interface HrciIdCardExportProps extends HrciIdCardFrontProps {
  /** Choose the rendering variant. 'standard' is CR80-friendly and supports orientation. */
  variant?: CardVariant;
  /** For 'standard' variant, choose orientation */
  orientation?: 'landscape' | 'portrait';
  /** Custom text for the top red band title (standard variant) */
  topTitle?: string;
  // Extra data supported by the 'standard' variant
  qrUri?: string;
  showQrBelowLogo?: boolean;
  ...
  widthInInches?: number; // physical width in inches (e.g., 3.375 for CR80 landscape)
  heightInInches?: number; // physical height in inches (e.g., 2.125 for CR80 landscape)
  bandHeightInches?: number; // header band height in inches for standard variant (e.g., 0.25)
  // Precise front section heights (standard variant only)
  topBandHeightInches?: number;
  blueBandHeightInches?: number;
  bodyHeightInches?: number;
  bottomBandHeightInches?: number;
  showSectionDebug?: boolean;
  // Landscape layout controls
  landscapePhotoSide?: 'left' | 'right';
  landscapeQrSide?: 'left' | 'right';
  qrSizePx?: number;
  // Landscape photo sizing controls
  landscapePhotoAspect?: number;
  landscapePhotoWidthInches?: number;
  landscapePhotoWidthPx?: number;
  landscapePhotoHeightInches?: number;
  landscapePhotoHeightPx?: number;
  landscapePhotoScale?: number;
  landscapePhotoTopOffsetPx?: number;
  showSignatureUnderPhoto?: boolean;
  landscapePhotoHorizontalOffsetPx?: number;
  landscapeStampScale?: number;
  landscapeStampPosition?: 'overlap' | 'above' | 'below';
  landscapeStampAboveScale?: number;
  landscapeStampBelowScale?: number;
  ...
  // If true, disable wide auto-scaling in the front standard component to keep export visuals identical to preview
  freezeWideScale?: boolean;
}
...
export const HrciIdCardExport = React.forwardRef<HrciIdCardExportHandle, HrciIdCardExportProps>(({ previewWidth = 360, exportWidth = 2160, showActions = true, ...cardProps }, ref) => {
  ...
  landscapeQrSide,
  qrSizePx,
  landscapePhotoAspect,
  landscapePhotoWidthInches,
  landscapePhotoWidthPx,
  landscapePhotoHeightInches,
  landscapePhotoHeightPx,
  landscapePhotoScale,
  landscapePhotoTopOffsetPx,
  showSignatureUnderPhoto,
  landscapePhotoHorizontalOffsetPx,
  landscapeStampScale,
  landscapeStampPosition,
  landscapeStampAboveScale,
  ...
  // Physical sizing (default to CR80 landscape if not specified)
  const defaultWIn = orientation === 'portrait' ? 2.125 : 3.375;
  const defaultHIn = orientation === 'portrait' ? 3.375 : 2.125;
  const physWIn = widthInInches ?? defaultWIn;
  const physHIn = heightInInches ?? defaultHIn;
  const targetAspectPhys = physHIn / physWIn; // height/width

  // Compute effective export width based on DPI/physical width when provided
  const effectiveExportWidth = (physWIn && dpi)
    ? Math.max(Math.round(physWIn * dpi), 300)
    : (typeof exportWidth === 'number' && exportWidth > 0 ? exportWidth : 1440);

  // Base card aspect (height/width) depends on variant
  // 'exact' currently designed as portrait-like; 'standard' can match the target CR80 aspect
  const BASE_ASPECT_EXACT = 3.375 / 2.125; // ~1.588 (portrait-like in Exact component code)
  const BASE_ASPECT_STANDARD = targetAspectPhys; // let standard match CR80 by default
  const BASE_ASPECT = variant === 'exact' ? BASE_ASPECT_EXACT : BASE_ASPECT_STANDARD;

  // Decide padding: if target aspect provided or padToCR80 or physical inches given, pad to target aspect
  const desiredAspect = targetAspect ?? (padToCR80 ? (3.375 / 2.125) : undefined) ?? targetAspectPhys;
  const padAspect = desiredAspect ?? BASE_ASPECT;
  const doPad = !!desiredAspect && Math.abs(padAspect - BASE_ASPECT) > 0.001;
  const exportHeightPx = Math.round(effectiveExportWidth * (doPad ? padAspect : BASE_ASPECT));
  // Derived physical height (not currently displayed separately; retained for potential future use)
  const heightInches = +( (physWIn * (doPad ? padAspect : BASE_ASPECT)).toFixed(3) ); // eslint-disable-line @typescript-eslint/no-unused-vars

  // Visible preview padding logic (so preview can match CR80 proportions like the back card)
  const previewPadAspect = previewPadToCR80 ? targetAspectPhys : (previewTargetAspect || BASE_ASPECT);
  const previewDoPad = !!previewPadToCR80 || (!!previewTargetAspect && Math.abs(previewTargetAspect - BASE_ASPECT) > 0.001) || (!!desiredAspect && Math.abs(desiredAspect - BASE_ASPECT) > 0.001);
    ...
    <View style={styles.root}>
      ...
      {disableTapToDownload ? (
        previewDoPad ? (
          <View style={{ width: previewWidth, height: previewHeightPx, alignItems: 'center', justifyContent: 'center' }}>
            {variant === 'standard' ? (
              <HrciIdCardFrontStandard {...(cardProps as any)} width={previewWidth} orientation={orientation} bandHeightInches={bandHeightInches} />
            ) : (
        ...
        >
          {previewDoPad ? (
            <View style={{ width: previewWidth, height: previewHeightPx, alignItems: 'center', justifyContent: 'center' }}>
              {variant === 'standard' ? (
              <HrciIdCardFrontStandard {...(cardProps as any)} width={previewWidth} orientation={orientation}
                bandHeightInches={bandHeightInches}
                topBandHeightInches={topBandHeightInches}
                blueBandHeightInches={blueBandHeightInches}
                bodyHeightInches={bodyHeightInches}
                bottomBandHeightInches={bottomBandHeightInches}
                showSectionDebug={showSectionDebug}
                  landscapePhotoSide={landscapePhotoSide}
                  landscapeQrSide={landscapeQrSide}
                  qrSizePx={qrSizePx}
                  landscapePhotoAspect={landscapePhotoAspect}
                  landscapePhotoWidthInches={landscapePhotoWidthInches}
                  landscapePhotoWidthPx={landscapePhotoWidthPx}
                  landscapePhotoHeightInches={landscapePhotoHeightInches}
                  landscapePhotoHeightPx={landscapePhotoHeightPx}
                  landscapePhotoScale={landscapePhotoScale}
                  landscapePhotoTopOffsetPx={landscapePhotoTopOffsetPx}
                  showSignatureUnderPhoto={showSignatureUnderPhoto}
                  landscapePhotoHorizontalOffsetPx={landscapePhotoHorizontalOffsetPx}
                  landscapeStampScale={landscapeStampScale}
                  landscapeStampPosition={landscapeStampPosition}
                  landscapeStampAboveScale={landscapeStampAboveScale}
                  landscapeStampBelowScale={landscapeStampBelowScale}
                  showQrBelowLogo={cardProps.showQrBelowLogo as any}
                  {...(freezeWideScale ? { wideScale: 1 } : {})}
              />
              ) : (
                <HrciIdCardFrontExact {...cardProps} width={previewWidth} />
              )}
      ...
      {showExportInfo && (
        <Text style={styles.exportInfo}>
          {desiredAspect ? 'Wallet card (CR80 or custom)' : 'Base aspect'}:
          {' '}
          {`${(physWIn * 25.4).toFixed(1)}mm`}
          {' \u00d7 '}
          {`${(physHIn * 25.4).toFixed(1)}mm`}
          ...
          {doPad ? (
            fitMode === 'pad' ? (
              <View style={{ width: effectiveExportWidth, height: Math.round(effectiveExportWidth * BASE_ASPECT), alignItems: 'center', justifyContent: 'center', backgroundColor: 'transparent' }}>
                {variant === 'standard' ? (
                  <HrciIdCardFrontStandard {...(cardProps as any)} width={effectiveExportWidth} orientation={orientation}
                    bandHeightInches={bandHeightInches}
                    topBandHeightInches={topBandHeightInches}
                    blueBandHeightInches={blueBandHeightInches}
                    bodyHeightInches={bodyHeightInches}
              ...
              (() => {
                ...
                return (
                  <View style={{ width: scaledW, height: scaledH, alignItems: 'center', justifyContent: 'center', backgroundColor: 'transparent' }}>
                    {variant === 'standard' ? (
                      <HrciIdCardFrontStandard {...(cardProps as any)} width={scaledW} orientation={orientation}
                        bandHeightInches={bandHeightInches}
                        topBandHeightInches={topBandHeightInches}
                        blueBandHeightInches={blueBandHeightInches}
                        bodyHeightInches={bodyHeightInches}
